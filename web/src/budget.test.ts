...
test("вес скриптов первой загрузки укладывается в бюджет 150 КБ", () => {
  for (const entry of [pageEntry(), showcaseEntry()]) {
    ...
    assertBudget(`${entry}: скрипты ${weight.toFixed(1)} КБ`, weight, SCRIPT_BUDGET_KB);
  }
});
...
test("число модулей первой загрузки укладывается в бюджет", () => {
  for (const entry of [pageEntry(), showcaseEntry()]) {
    const graph = moduleGraph(entry);
    assertBudget(`${entry}: модулей ${graph.files.length}`, graph.files.length, MODULE_COUNT_BUDGET);
  }
});
