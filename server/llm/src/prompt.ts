 ...
 * Проверяется реестром запретов в области `промпты`. Название методики в самом
 * открытом ответе — слово человека о своей жизни, и запрещать его человеку нельзя;
 * оно остаётся внутри конверта данных, а из выхода его убирает валидатор.
 *
