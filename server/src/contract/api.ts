 ...
 * Правила контракта:
 *
 * 1. Наружу уходят только блоки, полосы карты, двери, предложение, следующая
 *    порция, кризисное состояние и уточняющие. Значения координат, `confidence`,
 *    машинные коды и имена координат остаются на сервере — это проверяет `Wire<>`
 *    из `./wire.js`.
...
/**
 * Кризисное состояние страницы. Тексты и контакты — из `content/crisis.md`.
 * Поле `line` — номер или адрес линии, не `value`: так не пересекается со стеной координат.
 */
...
export interface PageStateDto {
  ...
  /**
   * Кризис: разбора нет, предложения нет. null — обычная страница.
   * Сервер всегда отдаёт поле; у моков интерфейса его может не быть.
   */
  crisis?: CrisisDto | null;
  /**
   * Уточняющие после добора, если порог среза не взят.
   * Сервер всегда отдаёт поле; у моков интерфейса его может не быть.
   */
