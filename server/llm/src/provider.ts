 ...
 *
 * Наружу смотрит интерфейс. В репозитории — заглушка и адаптер OpenRouter.
 * Тот же приём, что у базы (`server/src/db/driver.ts`): смена реализации — это
 * новый модуль, а не правка вызывающего кода.
 *
