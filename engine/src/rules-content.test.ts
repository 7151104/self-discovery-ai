...
import { fullMapQuestions } from "./full-map.js";
...
test("у добора полной карты столько вопросов, сколько банк не спросил лестницей", () => {
  ...
  assert.deepEqual(
    fullMapQuestions().map((question) => question.id).sort(),
    left.map((question) => question.id).sort(),
    "порции полной карты и остаток банка после лестницы разошлись",
  );
  const covered = new Set<number>();
  for (const question of rawContent.bank) {
    if (asked.has(question.id) || left.some((rest) => rest.id === question.id)) {
      for (const coordinate of question.coordinates) covered.add(coordinate);
    }
  }
  assert.deepEqual(
    [...covered].sort((a, b) => a - b),
    Array.from({ length: 16 }, (_, index) => index + 1),
    "лестница вместе с добором полной карты обязана закрывать все 16 координат",
  );
});
