 ...
 * зависеть ни от ключа, ни от интернета. Он умеет отдавать заготовленные ответы
 * по очереди, падать заданным отказом, зависать до прекращения по `signal` и
 * вести журнал вызовов. Без этого не проверить ни повторы, ни таймауты, ни
 * предел стоимости.
 *
 * Рабочий контур сервера его не поднимает: там по умолчанию стоит заглушка
 * (`stub-provider.ts`), которая собирает выход из задания. Фикстура `envelope()`
 * по-прежнему нужна тестам слоя — этот провайдер её и отдаёт.
 */
