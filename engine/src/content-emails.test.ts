 ...
 * живёт его содержание. Контакт спрашиваем после первой порции и его можно пропустить
 * (вопрос 6), поэтому у каждого письма обязательно названо место на странице.
...
test("файл писем честно говорит, что почты может не быть", () => {
  const source = repoFile("content/emails.md");
  assert.match(source, /вопрос 6/, "в файле нет ссылки на решение о почте");
  assert.match(source, /## Если контакт пропущен/, "нет раздела о том, что делать без адреса");
  assert.ok(
